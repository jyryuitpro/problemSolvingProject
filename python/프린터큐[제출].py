...
# 1 0           : 문서의 수 N(100이하) | 몇 번째로 인쇄되었는지 궁금한 문서가 현재 Queue의 어떤 위치에 있는지를 알려주는 M
# 5             : 다음줄에 N개 문서의 중요도

# 4 2           : 문서의 수 N(100이하) | 몇 번째로 인쇄되었는지 궁금한 문서가 현재 Queue의 어떤 위치에 있는지를 알려주는 M
# 1 2 3 4       : 다음줄에 N개 문서의 중요도

# 6 0           : 문서의 수 N(100이하) | 몇 번째로 인쇄되었는지 궁금한 문서가 현재 Queue의 어떤 위치에 있는지를 알려주는 M
# 1 1 9 1 1 1   : 다음줄에 N개 문서의 중요도
...
for i in range(test_case):
    ...
    # 0 1 2 3
    for j in range(len(sorted_importance)):
        if sorted_importance[j] == importance[m]:
            count += 1
        else:
            count += 1
